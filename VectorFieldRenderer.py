...
class VectorFieldRenderer(QgsFeatureRendererV2):
    ...
    #def __init__(self):
        print "Inside VectorFieldRenderer"
    ...
    def clone(self):
        clone = VectorFieldRenderer(self._standalone)
    ...
    def setSymbolSizeForFeature( self, feature, scaling ):
        ...
        if self._isvalid:
          try:
             for i in range(self.NFields):
                 if self._usedfield[i]:
                    ## Modifications for standalone application
                    if self._standalone : 
                       value[i] = float(feature[self._fieldno[i]].toPyObject())
                    else : 
